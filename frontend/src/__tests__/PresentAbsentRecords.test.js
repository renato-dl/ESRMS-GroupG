import React from 'react';
import {getRouterPropsForTest} from '../utils';
import {shallow} from 'enzyme';
import PresentAbsentRecords from '../components/TeacherClassesAbsence/PresentAbsentRecords/PresentAbsentRecords';
import {ApplicationStoreContext} from '../store';
describe('Testing PresentAbsentRecords component', () => {
  
  test('Test if component PresentAbsentRecords (teacher)is rendered', () => {
    shallow(
      <ApplicationStoreContext.Provider value={{state: {userID: '26ce21c0-8d32-41d1-8d07-b4994fa53edf'}}}>
      <PresentAbsentRecords {...getRouterPropsForTest()} />
      </ApplicationStoreContext.Provider>
    );
  });

});

