...
describe('Tests about classes', () => {
  ...
  test('It should return the classes of a teacher', async () =>{
    ...
    const getTeachingClasses = await Class.getTeachingClasses(insertTeacher.id);
...
test('It should throw an error when the passed teacher id is missing or invalid', async () =>{
  try{
    await Class.getTeachingClasses();
...
test('It should return a message saying no entity found', async () =>{
    const classes = await Class.getTeachingClasses("teacherIdNotExisting");
    expect(classes).toMatchObject({message: "No Entity found"});
});
